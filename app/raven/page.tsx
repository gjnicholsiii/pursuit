      ...
      if (q) {
        ...
        where.push(`(
          ...
          or exists(
            ...
            where rp.agency_id=a.id
              and (rp.full_name ilike '%'||${p}||'%'
                or coalesce(rp.title,'') ilike '%'||${p}||'%'
                or coalesce(rp.role_family,'') ilike '%'||${p}||'%'
                or coalesce(rp.email,'') ilike '%'||${p}||'%')
          )
          or exists(
            select 1 from opportunities oq
            where oq.agency_id=a.id
              and (oq.title ilike '%'||${p}||'%'
                or coalesce(oq.description,'') ilike '%'||${p}||'%'
                or coalesce(oq.solicitation_number,'') ilike '%'||${p}||'%')
          )
          or exists(
            select 1 from raven_relationships rr
            where rr.agency_id=a.id
              and (coalesce(rr.related_name,'') ilike '%'||${p}||'%'
                or rr.relationship_type ilike '%'||${p}||'%'
                or coalesce(rr.evidence->>'excerpt','') ilike '%'||${p}||'%')
          )
        )`);
      }
      ...
      const [rawResult, rawCountResult] = await Promise.all([
        sql.query(
          `select
            ...
            a.county,
            a.website,
            count(o.id) filter(where o.status='open' and (o.due_at is null or o.due_at>=now()))::int open_opportunities,
            count(o.id)::int total_opportunities,
            max(o.last_seen_at)::text latest_activity,
            (select count(*)::int from raven_people rp where rp.agency_id=a.id) people_count,
            coalesce((
              select jsonb_agg(
                jsonb_build_object(
            ...
            coalesce((
              ...
              from (
                select * from (
                  select
                    rr.related_name as title,
                    rr.related_url as source_url,
                    null::text as due_at,
                    rr.last_seen_at::text as occurred_at,
                    'signal'::text as status,
                    coalesce(rr.evidence->>'signal_type','public_signal') as kind,
                    nullif(rr.evidence->>'excerpt','') as excerpt,
                    rr.last_seen_at as event_at
                  from raven_relationships rr
                  where rr.agency_id=a.id
                    and rr.relationship_type='buying_signal'
                  ...
                  select
                    ...
                    os.source_url,
                    os.due_at::text,
                    os.last_seen_at::text,
                    os.status,
                    'active_solicitation'::text,
                    null::text,
                    os.last_seen_at
                  from opportunities os
                  where os.agency_id=a.id
                    and os.status='open'
                    and (os.due_at is null or os.due_at>=now())
                ) signal_union
                order by event_at desc
                limit 6
              ) s
            ),'[]'::jsonb) recent_signals
          from agencies a
          left join opportunities o on o.agency_id=a.id
          ${clause}
          group by a.id
          order by
            (select count(*) from raven_relationships rr where rr.agency_id=a.id and rr.relationship_type='buying_signal') desc,
            (select count(*) from raven_people rp where rp.agency_id=a.id) desc,
        ...
        <header className="topbar">
          <form className="searchbox" action="/raven" method="get">
            ...
            <input
              name="q"
              defaultValue={q}
              placeholder="Search organization, buyer, trigger, email or solicitation..."
              style={{ width: "100%", border: 0, background: "transparent", outline: 0, color: "inherit", font: "inherit" }}
            />
        ...
        <div className="content">
          ...
          <section className="readiness-panel">
            <div className="readiness-copy">
              ...
              <h2>Choose the market.</h2>
              <p>Live verticals search organizations, buyers, public buying signals and procurement activity together.</p>
            </div>
            <div className="readiness-grid">
              {RAVEN_VERTICALS.map((v) => (
                <Link key={v.key} href={buildHref(v.key)} className="readiness-item" style={{ textDecoration: "none" }}>
                  <div>
                    <strong>{v.label.toUpperCase()} · {v.status.toUpperCase()}</strong>
                    <small>{v.buyerRoles.slice(0, 4).join(" · ")}</small>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <section className="section-block">
            <div className="section-heading">
              <div>
                <span>{vertical.label.toUpperCase()} INTELLIGENCE</span>
                <h2>{Number(counts.total).toLocaleString()} organizations indexed</h2>
              </div>
              <div>
                <strong>{Number(counts.enriched).toLocaleString()} enriched · {Number(counts.people).toLocaleString()} people</strong>
                <small style={{ display: "block" }}>{Number(counts.with_website).toLocaleString()} organizations with website data</small>
              </div>
            </div>
            ...
            {vertical.organizationTypes.length === 0 ? (
              <div className="readiness-panel">
                <div className="readiness-copy">
                  ...
                  <h2>{vertical.label} universe</h2>
                  <p>{vertical.description}</p>
                  <p><strong>Buyer roles:</strong> {vertical.buyerRoles.join(" · ")}</p>
                  <p><strong>Sources:</strong> {vertical.sourcePlan.join(" · ")}</p>
                </div>
              </div>
            ) : dataError ? (
              <div className="readiness-panel">
                <div className="readiness-copy">
                  <h2>Raven data needs attention.</h2>
                  <p>{dataError}</p>
                </div>
              </div>
            ) : (
              <div className="opportunity-list">
                  ...
                  return (
                    <article key={row.id} className="readiness-panel" style={{ marginBottom: 0 }}>
                      <div className="readiness-copy">
                          ...
                          <div style={{ marginTop: 14 }}>
                            ...
                            <div className="readiness-grid">
                              {signals.map((signal, index) => (
                                <div className="readiness-item" key={`${signal.kind}-${signal.title}-${index}`}>
                                  <div>
                        ...
                        {!people.length && (
                          <div className="readiness-item" style={{ marginTop: 14 }}>
                            <div><strong>BUYERS TO FIND</strong><small>{vertical.buyerRoles.slice(0, 6).join(" · ")}</small></div>
                          </div>
                        )}
                      </div>
                    </article>
                ...
                {!rows.length && (
                  <div className="readiness-panel">
                    <div className="readiness-copy"><h2>No organizations matched this search.</h2><p>Widen the search or switch verticals.</p></div>
                  </div>
                )}
              </div>
            )}
