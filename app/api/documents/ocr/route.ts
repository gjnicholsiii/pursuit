...
async function processOne(job: OcrJob) {
  ...
  try {
    if (Number(job.bytes) > OCR_MAX_BYTES) {
      await sql.query(
      ...
      return { ok: false, documentId: job.id, reason: "ocr_document_too_large", permanent: true };
    }
    ...
    // Keep one OCR worker per serverless invocation. Procurement scans are typically
    // English and this avoids multiplying WASM/model memory under bursty cron load.
    ...
    await sql.query(
      `insert into extracted_facts
       ...
       values(
        $1::uuid,$2::uuid,'document_text_extract',
        jsonb_build_object('text_storage_key',$3::text,'character_count',$4::int,'ocr',true,'truncated',$5::boolean),
        null,jsonb_build_object('document_id',$2::text,'method','ocr'),0.85
       )
...
export async function GET(request: NextRequest) {
  ...
  await sql.query(`
    update document_jobs
       set state=case when attempts>=max_attempts then 'dead' else 'pending' end,
           run_after=now()+(interval '1 second'*least(900,power(2,attempts))),
           leased_until=null,lease_owner=null,last_error=coalesce(last_error,'ocr lease expired'),updated_at=now()
  ...
  const rows = await sql.query(`
    with claim as (
      ...
      update document_jobs j
         set state='leased',leased_until=now()+interval '10 minutes',lease_owner=$1,attempts=attempts+1,updated_at=now()
        from claim
       where j.id=claim.id
      returning j.id as job_id,j.document_id,j.host_class,j.priority,j.meta
    )
    select leased.job_id::text,d.id,d.opportunity_id,d.storage_key,leased.host_class,leased.priority,
           coalesce((leased.meta->>'bytes')::bigint,0)::bigint as bytes
    from leased
    join opportunity_documents d on d.id=leased.document_id
  `, [owner]) as OcrJob[];
