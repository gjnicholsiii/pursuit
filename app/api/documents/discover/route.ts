...
function scanUrls(opp:OpportunityRow){const urls=new Set<string>([opp.source_url]);const raw=opp.raw_payload||{};for(const key of ["sourcePage","detailUrl","detailURL","bidUrl","bidURL","url"]){const value=raw[key];if(typeof value==="string"&&isHttp(value))urls.add(value)}if(opp.adapter_key.startsWith("periscope_")&&opp.external_id){try{const base=new URL(opp.source_url);const root=`${base.protocol}//${base.host}/bso/`;const detail=new URL("external/bidDetail.sda",root);detail.searchParams.set("docId",opp.external_id);detail.searchParams.set("external","true");detail.searchParams.set("parentUrl","close");urls.add(detail.toString())}catch{}}return [...urls].slice(0,4)}
  ...
  const rows=await sql.query(`
    with candidates as (
      select o.id,o.title,o.source_url,o.external_id,o.raw_payload,o.due_at,a.agency_type,s.source_name,s.adapter_key,
        row_number() over (partition by s.adapter_key order by coalesce((o.raw_payload->>'pursuitDocumentScannedAt')::timestamptz,'epoch'::timestamptz),o.due_at asc nulls last,o.last_seen_at desc) as family_rank,
        case
