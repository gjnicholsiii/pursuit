...
function safeName(url: string, fallback = "document") { try { const name=decodeURIComponent(new URL(url).pathname.split("/").filter(Boolean).pop()||fallback); return name.replace(/[^a-zA-Z0-9._() -]+/g,"-").replace(/\s+/g," ").trim()||fallback; } catch { return fallback; } }
...
async function inspectHtml(html:string,pageUrl:string,opp:OpportunityRow,discovered:Set<string>,follow:Set<string>){const $=cheerio.load(html);const ids=identifiers(opp).map(v=>v.toLowerCase());const titleWords=norm(opp.title).split(" ").filter(v=>v.length>4).slice(0,5);$("a,button,[onclick],[data-url],[data-href]").each((_,el)=>{const node=$(el);const text=node.text().replace(/\s+/g," ").trim();const hay=(text+" "+(node.attr("href")||"")+" "+(node.attr("onclick")||"")).toLowerCase();const values=[node.attr("href"),node.attr("data-url"),node.attr("data-href")].filter((v):v is string=>Boolean(v));values.push(...extractQuotedUrls(node.attr("onclick")||"",pageUrl));const oppSpecific=ids.some(id=>hay.includes(id))||titleWords.filter(word=>hay.includes(word)).length>=3;for(const raw of values){if(raw.startsWith("javascript:")){for(const extracted of extractQuotedUrls(raw,pageUrl)){if(FILE_EXT.test(extracted)||DOC_URL_HINT.test(extracted))discovered.add(extracted);else if(DOC_TEXT_HINT.test(text)||oppSpecific)follow.add(extracted)}continue}const url=absolute(pageUrl,raw);if(!url)continue;if(FILE_EXT.test(url))discovered.add(url);else if(oppSpecific)follow.add(url);else if(DOC_URL_HINT.test(url)&&DOC_TEXT_HINT.test(text||url))follow.add(url);else if(DOC_TEXT_HINT.test(text))follow.add(url)}});for(const match of html.matchAll(/https?:\/\/[^"'<>\s]+/g)){const url=match[0].replace(/&amp;/g,"&");if(FILE_EXT.test(url))discovered.add(url)}}
async function fetchAndInspect(url:string,opp:OpportunityRow,discovered:Set<string>,follow:Set<string>,timeoutMs=9000){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),timeoutMs);try{const response=await fetch(url,{redirect:"follow",signal:controller.signal,cache:"no-store",headers:{"User-Agent":"Mozilla/5.0 PursuitGovernmentRevenue/1.0",Accept:"text/html,application/pdf,application/octet-stream,*/*"}});if(!response.ok)return response.status;const contentType=(response.headers.get("content-type")||"").toLowerCase();if(contentType.includes("application/pdf")||FILE_EXT.test(response.url)){discovered.add(response.url);return response.status}await inspectHtml(await response.text(),response.url,opp,discovered,follow);return response.status}catch{return 0}finally{clearTimeout(timeout)}}
