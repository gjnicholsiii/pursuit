...
async function failJob(jobId: string, error: string, permanent = false) {
  ...
  await sql.query(
    permanent
      ? `update document_jobs set state='dead',attempts=max_attempts,leased_until=null,lease_owner=null,last_error=$2,updated_at=now() where id=$1::bigint`
      : `update document_jobs set state=case when attempts>=max_attempts then 'dead' else 'pending' end,run_after=now()+(interval '1 second'*least(600,power(2,attempts))),leased_until=null,lease_owner=null,last_error=$2,updated_at=now() where id=$1::bigint`,
...
function cookieHeader(headers: Headers) {
  ...
  for (const value of values) {
    ...
    for (const match of matches) {
      ...
      if (cleaned && !cookies.includes(cleaned)) cookies.push(cleaned);
    }
  }
  return cookies.join("; ");
}

async function fetchIonWave(document: PendingDocumentRow, signal: AbortSignal, baseHeaders: Record<string, string>) {
  const bootstrap = await fetch(document.opportunity_source_url, {
    redirect: "follow",
    signal,
  ...
  const wanted = normalizedName(document.filename);
  let freshUrl = document.source_url;
  ...
  for (const row of rows) {
    ...
    const rawName = firstCell.text().replace(/\(please login to view this document\)/i, "").trim();
    if (normalizedName(rawName) !== wanted) continue;
    const href = firstCell.find("a[href]").first().attr("href");
    if (!href) continue;
    try { freshUrl = new URL(href, bootstrap.url || document.opportunity_source_url).toString(); } catch { /* keep stored URL */ }
    break;
  }

  if (freshUrl !== document.source_url) {
    const sql = getSql();
    await sql.query(`update opportunity_documents set source_url=$2 where id=$1::uuid`, [document.id, freshUrl]);
  }

  const headers: Record<string, string> = {
    ...baseHeaders,
    Referer: bootstrap.url || document.opportunity_source_url,
    Accept: "application/pdf,application/octet-stream,*/*",
  };
  if (cookie) headers.Cookie = cookie;
  return fetch(freshUrl, { redirect: "follow", signal, headers, cache: "no-store" });
}
...
async function fetchDocument(document: PendingDocumentRow, signal: AbortSignal) {
  ...
  if (document.host_class === "ionwave") {
    try { return await fetchIonWave(document, signal, baseHeaders); }
    catch { return fetch(document.source_url, { redirect: "follow", signal, headers: { ...baseHeaders, Referer: document.opportunity_source_url }, cache: "no-store" }); }
  }
  return fetch(document.source_url, { redirect: "follow", signal, headers: baseHeaders, cache: "no-store" });
}
...
async function acquireOne(document: PendingDocumentRow) {
  ...
  try {
    ...
    if (!looksValid(bytes, contentType, filename)) {
      ...
      await failJob(document.job_id, "invalid_payload", true);
      return { ok: false, documentId: document.id, status: 422 };
    }
    ...
    await failJob(document.job_id, message, false);
...
export async function GET(request: NextRequest) {
  ...
  await sql.query(`insert into document_jobs(document_id,stage,host_class,priority)
    ...
    greatest(0,100-case when o.due_at between now() and now()+interval '30 days' then 40 else 0 end-case when lower(d.filename)~'(sow|pws|statement.?of.?work|scope|specification|rfp|rfq|ifb|bid.?form|pricing|cost.?proposal|evaluation|addend|amend|questions?.?and.?answers?)' then 25 else 0 end+case when lower(d.filename)~'(w.?9|vendor.?registration|sample.?contract)' then 25 else 0 end)
    from opportunity_documents d join opportunities o on o.id=d.opportunity_id join sources s on s.id=o.source_id
    where d.extraction_status='pending' and d.storage_key is null and d.is_missing=false and o.status='open' and (o.due_at is null or o.due_at>=now())
