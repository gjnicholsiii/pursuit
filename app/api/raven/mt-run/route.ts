...
export async function GET(req: NextRequest) {
  if (req.nextUrl.searchParams.get("run") !== "montana-authoritative-20260907") return NextResponse.json({ok:false},{status:404});
  ...
  const authoritative = await invoke('/api/cron/raven-montana-authoritative');
  const verify = authoritative.status===200 ? await invoke('/api/cron/raven-montana-verify-authoritative') : null;
