...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const opportunity = await sql.query(
    `select o.id,s.adapter_key,s.source_family
     from opportunities o join sources s on s.id=o.source_id
     where o.id=$1 and o.status='open' and (o.due_at is null or o.due_at>=now()) limit 1`,
    [id],
  ...
  const documents = await sql.query(
    `select id, filename, document_type, fetched_at, extraction_status
     ...
     order by
       ...
       published_at desc nulls last, filename asc
     limit 8`,
    [id],
      ...
      await sql.query(
        `insert into document_jobs (document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta)
         ...
         on conflict(document_id,stage) do update set
           state='pending',host_class=excluded.host_class,priority=0,run_after=now(),leased_until=null,lease_owner=null,
  ...
  try {
    const response=await fetch(new URL("/api/documents/acquire",workerOrigin(request)),{
      cache:"no-store",headers:{authorization:`Bearer ${secret}`},signal:AbortSignal.timeout(240_000),
    });
