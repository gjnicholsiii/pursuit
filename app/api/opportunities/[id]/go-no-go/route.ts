...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const document of documents) {
    if (!document.fetched_at) {
      await sql.query(`insert into document_jobs(document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta) values($1,'acquire',$2,0,'pending',0,5,now(),jsonb_build_object('reason','go_no_go','organizationId',$3,'opportunityId',$4)) on conflict(document_id,stage) do update set state=case when document_jobs.state in ('done','leased') then document_jobs.state else 'pending' end,host_class=excluded.host_class,priority=0,run_after=case when document_jobs.state='leased' then document_jobs.run_after else now() end,leased_until=case when document_jobs.state='leased' then document_jobs.leased_until else null end,lease_owner=case when document_jobs.state='leased' then document_jobs.lease_owner else null end,attempts=case when document_jobs.state='dead' then 0 else document_jobs.attempts end,meta=coalesce(document_jobs.meta,'{}'::jsonb)||excluded.meta,updated_at=now()`,[document.id,hostClass,profile.organizationId,id]);
      continue;
    }
    ...
    if (Number(document.analyzer_version||0) < REQUIRED_ANALYZER_VERSION) {
      await sql.query(`insert into document_jobs(document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta) values($1,'analyze',$2,0,'pending',0,5,now(),jsonb_build_object('reason','go_no_go','organizationId',$3,'opportunityId',$4,'requiredAnalyzerVersion',$5)) on conflict(document_id,stage) do update set state=case when document_jobs.state='leased' then 'leased' else 'pending' end,host_class=excluded.host_class,priority=0,run_after=case when document_jobs.state='leased' then document_jobs.run_after else now() end,leased_until=case when document_jobs.state='leased' then document_jobs.leased_until else null end,lease_owner=case when document_jobs.state='leased' then document_jobs.lease_owner else null end,attempts=case when document_jobs.state='leased' then document_jobs.attempts else 0 end,last_error=case when document_jobs.state='leased' then document_jobs.last_error else null end,meta=coalesce(document_jobs.meta,'{}'::jsonb)||excluded.meta,updated_at=now()`,[document.id,hostClass,profile.organizationId,id,REQUIRED_ANALYZER_VERSION]);
    }
  }
