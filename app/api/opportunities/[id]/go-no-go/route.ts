...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const opportunity = await sql.query(
    `select o.id,s.adapter_key,s.source_family from opportunities o join sources s on s.id=o.source_id
     where o.id=$1 and o.status='open' and (o.due_at is null or o.due_at>=now()) limit 1`, [id]
  ) as Array<{ id:string; adapter_key:string; source_family:string }>;
  ...
  const documents = await sql.query(
    `select id,filename,document_type,fetched_at,extraction_status from opportunity_documents
     ...
     order by case
       ...
       when lower(coalesce(document_type,'')) ~ '(solicitation|spec|addendum|amendment)' then 3 else 9 end,
    ...
    if (!document.fetched_at) {
      await sql.query(
        `insert into document_jobs(document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta)
         values($1,'acquire',$2,0,'pending',0,5,now(),jsonb_build_object('reason','go_no_go','organizationId',$3,'opportunityId',$4))
         on conflict(document_id,stage) do update set state='pending',host_class=excluded.host_class,priority=0,run_after=now(),leased_until=null,lease_owner=null,
        ...
        [document.id,hostClass,profile.organizationId,id]
      );
    } else if (!['complete','extracted','analyzed','text_extracted'].includes(document.extraction_status || '')) {
      await sql.query(
        `insert into document_jobs(document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta)
         values($1,'extract',$2,0,'pending',0,5,now(),jsonb_build_object('reason','go_no_go','organizationId',$3,'opportunityId',$4))
         on conflict(document_id,stage) do update set state=case when document_jobs.state='done' then 'done' else 'pending' end,
         host_class=excluded.host_class,priority=0,run_after=now(),leased_until=null,lease_owner=null,
         meta=coalesce(document_jobs.meta,'{}'::jsonb)||excluded.meta,updated_at=now()`,
        [document.id,hostClass,profile.organizationId,id]
      );
    }
