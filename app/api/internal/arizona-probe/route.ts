...
function parsePage(html: string) {
  ...
  const rows = $("#body_x_grid_grd tbody tr").toArray().map(row => {
    ...
    return {
      internalId: href.match(/\/bpm\/process_manage_extranet\/(\d+)/)?.[1] || null,
      linkText: link.text().replace(/^Edit\s+/i, "").replace(/\s+/g, " ").trim(),
      cells,
    };
  });
  return {
...
export async function GET() {
  try {
    ...
    const initial = parsePage(html);
    const pages: ReturnType<typeof parsePage>[] = [initial];

    for (let page = 1; page <= initial.maxPage; page += 1) {
      const next = await fetchPage(page, html, cookies);
      html = next.html;
      cookies = next.cookies;
      const parsed = parsePage(html);
      if (parsed.currentPage !== page) throw new Error(`Arizona APP expected page ${page}, received ${parsed.currentPage}`);
      pages.push(parsed);
    }
    ...
    const widthCounts = allRows.reduce<Record<string, number>>((acc, row) => {
      acc[String(row.cells.length)] = (acc[String(row.cells.length)] || 0) + 1;
      return acc;
    }, {});
    const cellValuesByIndex: Record<string, string[]> = {};
    for (const row of allRows) row.cells.forEach((cell, index) => {
      const list = cellValuesByIndex[String(index)] ||= [];
      if (cell && !list.includes(cell) && list.length < 30) list.push(cell);
    });

    return NextResponse.json({
      ok: allRows.length === initial.totalRows && uniqueIds.size === initial.totalRows,
