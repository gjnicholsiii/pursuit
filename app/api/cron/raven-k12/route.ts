...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Official-site reconciliation runs independently. Keep this worker focused on
    // one district enrichment pass so external district-site latency cannot consume
    // the same 300-second function budget twice.
    ...
    return NextResponse.json({ ok: true, ...result, falsePeopleRemoved: removed.length });
  } catch (error) {
