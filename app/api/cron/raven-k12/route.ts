...
function plausibleName(v:string){
  const s=clean(v.replace(EMAIL_RE," ").replace(PHONE_RE," ").replace(/[^A-Za-z.' -]/g," "));
  if(s.length<5||s.length>70)return null;
  const parts=s.split(" ").filter(Boolean);
  if(parts.length<2||parts.length>5)return null;
  if(/director|superintendent|technology|security|safety|board|school|district|department|office|contact|email|phone|services|president|chair|trustee/i.test(s))return null;
  return parts.every(p=>/^[A-Za-z][A-Za-z.'-]*$/.test(p))?s:null;
}
async function fetchHtml(url:string){
  const c=new AbortController();const t=setTimeout(()=>c.abort(),FETCH_TIMEOUT);
  try{const r=await fetch(url,{redirect:"follow",cache:"no-store",signal:c.signal,headers:{"user-agent":"Mozilla/5.0 (compatible; Pursuit-Raven/3.0; public-directory-research)",accept:"text/html,application/xhtml+xml"}});if(!r.ok)return null;const type=(r.headers.get("content-type")||"").toLowerCase();if(!type.includes("html"))return null;return{html:await r.text(),url:r.url||url};}catch{return null;}finally{clearTimeout(t);}
}
function links(base:URL,html:string){
  const $=cheerio.load(html);const scored=new Map<string,number>();
  $("a[href]").each((_,el)=>{const href=$(el).attr("href")||"";const label=clean($(el).text());try{const u=new URL(href,base);u.hash="";if(!sameSite(base,u)||!/^https?:$/.test(u.protocol))return;const text=`${label} ${u.pathname}`;if(!LINK_RX.test(text))return;let score=10;if(/staff|directory|leadership|administration|cabinet/i.test(text))score+=10;if(/superintendent|technology|security|safety|board/i.test(text))score+=20;scored.set(u.toString(),Math.max(scored.get(u.toString())||0,score));}catch{}});
  for(const p of COMMON){const u=new URL(p,base).toString();scored.set(u,Math.max(scored.get(u)||0,15));}
  return[...scored.entries()].sort((a,b)=>b[1]-a[1]).slice(0,16).map(([u])=>u);
}
function extract(html:string,sourceUrl:string,needed:Set<string>){
  const $=cheerio.load(html);const out:any[]=[];const nodes=$("article,li,tr,.staff,.staff-member,.staff-card,.person,.employee,.contact,.directory-item,.card,.profile,section,div").toArray();
  for(const el of nodes){
    const node=$(el);const text=clean(node.text());if(text.length<8||text.length>850)continue;
    for(const role of needed){
      const rx=ROLE_RX[role];if(!rx||!rx.test(text))continue;
      const match=text.match(rx)?.[0]||"";if(!match||BANNED.test(match))continue;
      const email=(node.find('a[href^="mailto:"]').first().attr("href")||"").replace(/^mailto:/i,"").split("?")[0]||text.match(EMAIL_RE)?.[0]||null;
      const phone=(node.find('a[href^="tel:"]').first().attr("href")||"").replace(/^tel:/i,"")||text.match(PHONE_RE)?.[0]||null;
      const candidates=[node.find(".name,.staff-name,.employee-name,.person-name,h1,h2,h3,h4,strong,b").first().text(),node.find("a").filter((_,a)=>!/^mailto:|^tel:/i.test($(a).attr("href")||"")).first().text()];
      let fullName:null|string=null;for(const c of candidates){fullName=plausibleName(c);if(fullName)break;}
      if(!fullName&&email)fullName=plausibleName(email.split("@")[0].replace(/[._-]+/g," "));
      if(!fullName)continue;
      let title=clean(match);
      const rawLines=node.text().split(/\r?\n|\||•/).map(clean).filter(Boolean);
      const exactLine=rawLines.find(line=>rx.test(line)&&line.length<=140&&!BANNED.test(line));if(exactLine)title=exactLine;
      out.push({role,fullName,title,email,phone,sourceUrl,score:(email?4:0)+(phone?2:0)+(exactLine?2:0)});
    }
  }
  const best=new Map<string,any>();for(const c of out){const old=best.get(c.role);if(!old||c.score>old.score)best.set(c.role,c);}return[...best.values()];
}

export async function GET(req:NextRequest){
  const auth=requireInternalAuth(req);if(auth)return auth;
  const started=Date.now();const sql=getSql();
  try{
    const agencies=await sql.query(`
      ...
      order by a.state_code,a.county,a.canonical_name
      limit $1
    `,[AGENCY_LIMIT]) as any[];

    let attempted=0,pages=0,found=0,promoted=0,failed=0;
    async function crawl(a:any){
      if(Date.now()-started>RUN_BUDGET)return;
      attempted++;const seed=safeUrl(String(a.website||""));if(!seed){failed++;return;}
      const home=await fetchHtml(seed.toString());if(!home){failed++;return;}
      const base=safeUrl(home.url)||seed;const needed=new Set<string>((a.roles||[]).map(String));const all:any[]=[];
      all.push(...extract(home.html,base.toString(),needed));pages++;
      const queue=links(base,home.html);
      for(const u of queue){if(Date.now()-started>RUN_BUDGET||all.length>=needed.size*3)break;const page=await fetchHtml(u);if(!page)continue;const final=safeUrl(page.url);if(!final||!sameSite(base,final))continue;pages++;all.push(...extract(page.html,final.toString(),needed));}
      const best=new Map<string,any>();for(const c of all){const old=best.get(c.role);if(!old||c.score>old.score)best.set(c.role,c);}
      for(const c of best.values()){
        ...
        if(rows.length){promoted+=rows.length;found++;}
      }
    }
    ...
    return NextResponse.json({ok:true,mode:'targeted-missing-slots',agenciesSelected:agencies.length,attempted,pagesScanned:pages,contactsFound:found,candidatesPromoted:promoted,failed,elapsedMs:Date.now()-started});
