...
async function texas(){
  const first=await fetch(TX,{cache:"no-store",redirect:"follow",headers:{"user-agent":"Mozilla/5.0 (compatible; Pursuit-Raven/5.2; authoritative-state-roster)",accept:"text/html,application/xhtml+xml"}});
  ...
  const post=await fetch(TX,{method:"POST",cache:"no-store",redirect:"follow",headers:{"user-agent":"Mozilla/5.0 (compatible; Pursuit-Raven/5.2; authoritative-state-roster)","content-type":"application/x-www-form-urlencoded","referer":TX,accept:"text/csv,text/plain,application/vnd.ms-excel,*/*"},body:body.toString()});
...
const SOURCES:Source[]=[{state:"TX",url:TX,fetch:texas}];
...
export async function GET(req:NextRequest){
  ...
  for(const source of SOURCES){
    ...
    let matched=0,filled=0; const touched=new Set<string>();
    for(const r of roster){ const rk=roleFor(r.title); const dk=districtKey(r.district); const s=slots.find(x=>!touched.has(x.id)&&x.role_key===rk&&(districtKey(x.canonical_name||"")===dk||districtKey(x.county||"")===dk||districtKey(x.canonical_name||"").includes(dk)||dk.includes(districtKey(x.canonical_name||"")))); if(!s)continue; matched++; touched.add(s.id); const u=await sql.query(`update raven_state_contacts set full_name=$2,title=$3,email=nullif($4,''),phone=nullif($5,''),source_url=$6,verification_status='candidate',evidence_note='Reachable contact ingested directly from authoritative statewide education directory; awaiting strict live revalidation.',updated_at=now() where id=$1 and verification_status='missing' returning id`,[s.id,r.fullName,r.title,r.email,r.phone,source.url]) as any[]; filled+=u.length; }
    results.push({state:source.state,source:source.url,fetched:roster.length,missingSlotsExamined:slots.length,districtsNewlyAdvanced:touched.size,matched,filled});
  }
