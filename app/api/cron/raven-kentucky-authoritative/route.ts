...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(SOURCE_URL, {
      ...
      headers: { "user-agent": "Mozilla/5.0 (compatible; Pursuit-Raven/4.0; authoritative-public-directory)", accept: "text/html,application/xhtml+xml" }
    });
  ...
  for (const row of rows) {
    ...
    `,[key,row.district,row.fullName,row.phone,SOURCE_URL]) as any[];
  ...
  return NextResponse.json({ok:true,source:SOURCE_URL,fetched:rows.length,newlyAttempted:matchedDistricts.length,filled,before:beforeRows[0],after:afterRows[0],matchedDistricts});
}
