...
export async function GET(req:NextRequest){
  ...
  async function crawl(a:Agency){if(Date.now()-started>RUN_BUDGET)return;attempted++;touched.add(a.id);const run=(await sql.query(`insert into raven_enrichment_runs(agency_id,status,diagnostics) values($1,'running',$2::jsonb) returning id::text`,[a.id,JSON.stringify({sourceClass:SOURCE_CLASS,website:a.website,state:STATE})]) as any[])[0];const runId=run?.id;const seed=safeUrl(a.website);if(!seed){blocked++;if(runId)await sql.query(`update raven_enrichment_runs set status='completed',completed_at=now(),diagnostics=coalesce(diagnostics,'{}'::jsonb)||$2::jsonb where id=$1`,[runId,JSON.stringify({result:'blocked_invalid_url'})]);return;}const home=await fetchHtml(seed.toString());if(!home){blocked++;if(runId)await sql.query(`update raven_enrichment_runs set status='completed',completed_at=now(),pages_scanned=0,people_found=0,diagnostics=coalesce(diagnostics,'{}'::jsonb)||$2::jsonb where id=$1`,[runId,JSON.stringify({result:'blocked_no_html'})]);return;}const base=safeUrl(home.url)||seed;const needed=new Set<string>((a.roles||[]).map(String));const hits:Hit[]=[];hits.push(...extract(home.html,base.toString(),needed));pages++;for(const u of links(base,home.html)){if(Date.now()-started>RUN_BUDGET||hits.length>=needed.size*3)break;const page=await fetchHtml(u);if(!page)continue;const final=safeUrl(page.url);if(!final||!sameSite(base,final))continue;pages++;hits.push(...extract(page.html,final.toString(),needed));}const best=new Map<string,Hit>();for(const h of hits){const old=best.get(h.role);if(!old||h.score>old.score)best.set(h.role,h);}for(const h of best.values()){const rows=await sql.query(`update raven_state_contacts set full_name=$2,title=$3,email=$4,phone=$5,source_url=$6,verification_status='candidate',evidence_note='Candidate discovered on current official Florida district website; no email inferred.',updated_at=now() where agency_id=$1 and role_key=$7 and verification_status='missing' returning id`,[a.id,h.fullName,h.title,h.email,h.phone,h.sourceUrl,h.role]) as any[];promoted+=rows.length;}if(runId)await sql.query(`update raven_enrichment_runs set status='completed',completed_at=now(),pages_scanned=$2,people_found=$3,diagnostics=coalesce(diagnostics,'{}'::jsonb)||$4::jsonb where id=$1`,[runId,Math.max(1,pages),best.size,JSON.stringify({result:best.size?'candidates_found':'exhausted',roles:[...needed]})]);}
  ...
  const remainingUntouched=(await sql.query(`select count(distinct c.agency_id)::int n from raven_state_contacts c join agencies a on a.id=c.agency_id where c.state_code=$1 and c.scope='district' and c.verification_status='missing' and c.role_key in ('assistant_superintendent','it_director','school_board','security_director') and a.website is not null and btrim(a.website)<>'' and not exists(select 1 from raven_enrichment_runs r where r.agency_id=a.id and r.status='completed' and r.diagnostics->>'sourceClass'=$2 and r.diagnostics->>'website'=a.website)`,[STATE,SOURCE_CLASS]) as any[])[0]?.n||0;
  return NextResponse.json({ok:true,state:STATE,mode:'durable-statewide-official-district-queue',sourceClass:SOURCE_CLASS,districtsSelected:agencies.length,districtsNewlyAttempted:touched.size,pagesScanned:pages,candidatesPromoted:promoted,blocked,remainingUntouched,before,after,net:{total:after.total-before.total,verified:after.verified-before.verified,candidate:after.candidate-before.candidate,missing:after.missing-before.missing,rejected:after.rejected-before.rejected},elapsedMs:Date.now()-started});
}
