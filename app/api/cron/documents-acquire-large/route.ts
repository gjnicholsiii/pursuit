...
export async function GET(request: NextRequest) {
  ...
  const sql = getSql();
  const rows = await sql.query(`select j.id::text job_id,d.id::text,d.opportunity_id::text,d.source_url,o.source_url opportunity_source_url,d.filename,s.source_family,j.host_class,j.priority,j.meta from document_jobs j join opportunity_documents d on d.id=j.document_id join opportunities o on o.id=d.opportunity_id join sources s on s.id=o.source_id where j.stage='acquire' and j.state='dead' and j.last_error in ('too_large_runtime_cap','oversize_deferred_external_processing','too_large_200mb') and d.storage_key is null and coalesce(d.is_missing,false)=false and o.status='open' and (o.due_at is null or o.due_at>=now()) order by j.priority,o.due_at asc nulls last,j.id limit 1`) as LargeDocumentRow[];
