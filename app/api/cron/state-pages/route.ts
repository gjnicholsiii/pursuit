...
export async function GET(request: NextRequest) {
  ...
  try {
    const results = await syncOfficialStatePages(false);
    const failures = results.filter(result => !result.ok);
    if (failures.length) console.warn("SLED_OFFICIAL_STATE_PAGES_PARTIAL", JSON.stringify(failures));
    return NextResponse.json({ ok: failures.length === 0, results, failures }, { status: failures.length ? 207 : 200 });
  } catch (error) {
    console.error("SLED_OFFICIAL_STATE_PAGES_FAILED", error);
