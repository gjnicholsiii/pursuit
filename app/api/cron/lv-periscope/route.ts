...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ ok: true, source: result.sourceName, state: result.stateCode, scanned: result.scanned, accepted: result.pursuits.length, stored, complete: result.complete });
}
