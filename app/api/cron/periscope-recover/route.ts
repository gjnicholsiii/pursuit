...
export async function GET(request:NextRequest){
  ...
  const sql=getSql();
  const rows=await sql.query(`select j.id::text job_id,d.id::text document_id,d.opportunity_id::text,d.filename,d.source_url document_url,o.source_url opportunity_url from document_jobs j join opportunity_documents d on d.id=j.document_id join opportunities o on o.id=d.opportunity_id join sources s on s.id=o.source_id where j.stage='acquire' and j.state='dead' and j.last_error='http_404' and s.adapter_key like 'periscope_%' and d.storage_key is null and coalesce(d.is_missing,false)=false and o.status='open' and (o.due_at is null or o.due_at>=now()) order by o.due_at asc nulls last,j.id limit ${BATCH}`) as Row[];
