...
export async function GET(request:NextRequest){
 ...
 for(const row of rows){
  ...
  try{
   ...
   if(response.status===429){status='rate_limited';await sql.query(`update opportunities set raw_payload=coalesce(raw_payload,'{}'::jsonb)||jsonb_build_object('pursuitSamPackageCheckedAt',now(),'pursuitPackageStatus',$2::text) where id=$1::uuid`,[row.id,status]);break}
   if(!response.ok){status=`source_http_${response.status}`;failed++}
   else{
 ...
 await sql.query(`insert into document_jobs(document_id,stage,host_class,priority,state,attempts,max_attempts,run_after,meta) select d.id,'acquire','sam',case when (o.title||' '||coalesce(o.description,''))~*'(access control|video surveillance|security system|security camera|cctv|fire alarm|nurse call|low voltage|structured cabling|intrusion|audiovisual|av systems)' then 0 else 2 end,'pending',0,5,now(),jsonb_build_object('reason','sam_proactive_recovery') from opportunity_documents d join opportunities o on o.id=d.opportunity_id where d.referenced_by='SAM.gov proactive package recovery' and d.storage_key is null and coalesce(d.is_missing,false)=false and o.status='open' on conflict(document_id,stage) do update set priority=least(document_jobs.priority,excluded.priority),state=case when document_jobs.state in('dead','skipped') then 'pending' else document_jobs.state end,attempts=case when document_jobs.state='dead' then 0 else document_jobs.attempts end,run_after=case when document_jobs.state in('dead','skipped') then now() else document_jobs.run_after end,updated_at=now()`);
 return NextResponse.json({ok:true,checked:rows.length,foundOpps,inserted,failed});
}
