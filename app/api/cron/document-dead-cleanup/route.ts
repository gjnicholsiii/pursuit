...
export async function GET(request:NextRequest){
  ...
  const terminal=await sql.query(`
    ...
    where j.document_id=d.id
      ...
      and (
        ...
        or o.status not in ('open','active','posted')
  ...
  const retired=await sql.query(`
    with doomed as (
      ...
      where j.stage='acquire' and j.state='dead'
        ...
        and o.status in ('open','active','posted') and (o.due_at is null or o.due_at>=now())
      limit 500
    ), updated as (
  ...
  await sql.query(`
    ...
    where o.status in ('open','active','posted') and (o.due_at is null or o.due_at>=now())
