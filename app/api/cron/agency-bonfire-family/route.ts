...
const ELIGIBLE_TYPES = ["k12", "higher_ed", "state_agency", "county", "municipal", "authority", "local_agency"];
...
async function fetchHtml(url: string) {
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "user-agent": "Mozilla/5.0 (compatible; Pursuit-Agency-Bonfire-Family/1.0)",
...
function extractBonfirePortals(html: string, pageUrl: string, agency: Agency) {
  ...
  $("a[href],iframe[src]").each((_, node) => {
    ...
    portals.set(slug, {
      slug,
      agencyName: agency.canonical_name,
      stateCode: agency.state_code.trim(),
      city: agency.city,
      agencyType: agency.agency_type,
      jurisdictionLevel: jurisdictionLevel(agency.agency_type),
    });
  });
...
export async function GET(request: NextRequest) {
  ...
  const rows = await sql.query(
    `select id::text, canonical_name, agency_type, state_code::text, city, website
     from agencies
     where agency_type = any($3::text[])
       and website is not null and website <> ''
       and state_code is not null
       and mod(abs(hashtextextended(id::text,17)),$1::bigint)=$2::bigint
     order by canonical_name`,
    [SHARD_COUNT, shard, ELIGIBLE_TYPES],
  ) as Agency[];
  ...
  const sync = Date.now() - started < RUN_BUDGET_MS
    ? await syncDiscoveredBonfirePortals([...portals.values()])
    : { portals: portals.size, succeeded: 0, failed: 0, stored: 0, diagnostics: [] };

  const agencyTypeCounts = rows.reduce<Record<string, number>>((acc, row) => {
    acc[row.agency_type] = (acc[row.agency_type] || 0) + 1;
    return acc;
  }, {});
  ...
  return NextResponse.json({
    ...
    agenciesSelected: rows.length,
    agencyTypeCounts,
    portalsDiscovered: portals.size,
    ...sync,
