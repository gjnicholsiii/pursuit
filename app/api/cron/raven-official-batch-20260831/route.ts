...
const records = [
  { state_code:"AZ", county:"Maricopa", org_hint:"Mesa Unified", role_key:"superintendent", full_name:"Dr. Matt Strom", title:"Superintendent", email:"superintendent@mpsaz.org", phone:"480-472-0200", source_url:"https://www.mpsaz.org/page/superintendency", evidence_note:"Current official Mesa Public Schools superintendency page lists Dr. Matt Strom as Superintendent and publishes the superintendent email and district phone." },
  { state_code:"AZ", county:"Maricopa", org_hint:"Mesa Unified", role_key:"security_director", full_name:"Jeff Solomon", title:"Director, School Safety & Security", email:null, phone:"480-472-0200", source_url:"https://departments.mpsaz.org/o/departments/page/security", evidence_note:"Current official Mesa Public Schools School Safety & Security page lists Jeff Solomon as Director. Direct email is not publicly exposed; district phone retained." },
  { state_code:"AZ", county:"Maricopa", org_hint:"Mesa Unified", role_key:"it_director", full_name:"David Sanders", title:"Chief Technology Officer", email:null, phone:"480-472-0200", source_url:"https://departments.mpsaz.org/o/departments/page/infosys", evidence_note:"Current official Mesa Public Schools Information Systems page lists David Sanders as Chief Technology Officer. Direct email is not publicly exposed; district phone retained." },
  { state_code:"AZ", county:"Maricopa", org_hint:"Mesa Unified", role_key:"school_board", full_name:"Courtney Davis", title:"Governing Board Member", email:null, phone:"480-472-0200", source_url:"https://www.mpsaz.org/article/1886869", evidence_note:"Official Mesa Public Schools 2024 election results list Courtney Davis as an elected Governing Board member beginning a four-year term in January 2025. Direct public email was not exposed on the current board page." },
...
export async function GET(req: NextRequest) {
  ...
  for (const r of records) {
    const agencies = await sql.query(`
      ...
      order by case when lower(canonical_name)=lower($3) then 0 else 1 end,id limit 1
    ...
    if(!updated.length){
      ...
      if(!exists.length){
        ...
        await sql.query(`update raven_state_contacts set agency_id=$1,updated_at=now() where id=$2`,[agencyId,exists[0].id]);
      }
    }
