...
type MsBid = {
  ...
  AdvertiseTime?: string | null;
  Agency?: string | null;
  AgencyNumber?: string | null;
  Attachments?: MsAttachment[] | null;
  ...
  SubProcurementCategoryDescription?: string | null;
  SubProcurementCategoryID?: string | null;
  VerNumber?: string | null;
};
  ...
  try {
    ...
    const records = rows.flatMap(row => {
      ...
      const record: SledOpportunityRecord = {
        ...
        agency: {
          key: `mississippi-procurement:${row.AgencyNumber || agencyName}`,
          name: agencyName,
          agencyType: type,
          jurisdictionLevel: type === "state_agency" ? "state" : "local",
          stateCode: "MS",
          website: BOARD_URL,
        },
        title,
        ...
        sourceUrl: BOARD_URL,
        rawPayload: {
          platform: "Mississippi MAGIC / MS.gov Procurement Search",
          bidId: row.BidID,
          bidNumber: row.BidNumber,
          objectId: row.ObjectID || null,
          bidStatus: row.BidStatus || null,
          bidType: row.BidType || null,
          agency: row.Agency || null,
          agencyNumber: row.AgencyNumber || null,
          buyerName: row.BuyerName || null,
          buyerEmail: row.BuyerEmail || null,
          buyerPhone: row.BuyerPhone || null,
          category: row.ProcurementCategoryDescription || null,
          categoryId: row.ProcurementCategoryID || null,
          subcategory: row.SubProcurementCategoryDescription || null,
          subcategoryId: row.SubProcurementCategoryID || null,
    ...
    const persisted = await persistSledOpportunities(SOURCE, records, {
      mode: "mississippi_public_open_refresh",
      recordChanges: true,
      closeMissing: true,
    });
    return {
