...
const SOURCE: SledSourceConfig = {
  adapterKey: "cgi_advantage_legacy_me",
...
export interface MaineSyncResult {
  stateCode: "ME";
  sourceName: string;
  ok: boolean;
  rowsFound: number;
...
function documentRefs(html: string) {
  ...
  return match
    ? [...match[1].matchAll(/['"]([^'"]+)['"]/g)].map(item => text(item[1])).filter(Boolean)
    : [];
}
...
function parseEastern(value: string) {
  const cleaned = text(value).replace(/\bEDT\b|\bEST\b/i, "").trim();
...
function parsePage(html: string) {
  ...
  if (refs.length !== rows.length) {
    throw new Error(`Maine VSS row/reference mismatch: ${rows.length} rows / ${refs.length} references`);
  }
  ...
  return rows.map((row, index): SledOpportunityRecord | null => {
    ...
    const agencyName = agencyLines[0] || "State of Maine";
    ...
    return {
      ...
      sourceUrl: ENTRY,
      rawPayload: {
        platform: "CGI Advantage VSS AltSelfService",
...
async function establishOpenSession() {
  ...
  if (!first.ok) throw new Error(`Maine VSS entry returned ${first.status}`);
  ...
  if (!guest.ok) throw new Error(`Maine VSS guest login returned ${guest.status}`);
  ...
  const startupUrl = new URL($guest("frame[name='Startup']").attr("src") || "", base).toString();
  const startup = await fetch(startupUrl, {
    headers: { accept: "text/html", "user-agent": UA, referer: guest.url, ...(cookies.length ? { cookie: cookies.join("; ") } : {}) },
    cache: "no-store",
  });
  if (!startup.ok) throw new Error(`Maine VSS startup returned ${startup.status}`);
  ...
  if (!search.ok) throw new Error(`Maine VSS solicitation search returned ${search.status}`);
  ...
  if (!open.ok) throw new Error(`Maine VSS open solicitations returned ${open.status}`);
...
async function fetchCompleteSweep() {
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    ...
    if (!pageRecords.length) throw new Error(`Maine VSS page ${page} returned no open solicitation rows`);
    for (const record of pageRecords) records.set(record.externalId, record);
    pagesFetched = page;

    if (nextDisabled(html)) {
      complete = true;
      break;
    }
    ...
    if (!next.ok) throw new Error(`Maine VSS page ${page + 1} returned ${next.status}`);
    html = await next.text();
    cookies = mergeCookies(cookies, cookiePairs(next));
  }

  if (!complete) throw new Error(`Maine VSS did not reach the final page within ${MAX_PAGES} pages`);
  return { records: [...records.values()], pagesFetched, complete };
}

export async function syncMaineLegacyVss(): Promise<MaineSyncResult> {
  try {
    const sweep = await fetchCompleteSweep();
    const persisted = await persistSledOpportunities(SOURCE, sweep.records, {
      mode: "maine_legacy_vss_refresh",
      recordChanges: true,
      closeMissing: true,
    });
    return {
