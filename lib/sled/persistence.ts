...
export async function persistSledOpportunities(
  ...
  options: { mode?: string; recordChanges?: boolean } = {},
): Promise<SledPersistenceResult> {
  if (!opportunities.length) return { stored: 0, newRecords: 0, changedRecords: 0 };
  const sql = getSql();
  ...
  await sql.query(
    ...
    [sourceId, records.length, newRecords, changed.length, JSON.stringify({ adapter: source.adapterKey, mode: options.mode || "interactive" })],
  );
  await sql.query(`update sources set last_success_at=now(), last_error=null, health_score=100 where id=$1`, [sourceId]);

  return { stored: records.length, newRecords, changedRecords: changed.length };
}
