...
export async function consolidateExactK12Duplicates() {
  ...
  const ravenPreservingRows = await sql.query(`
    ...
    ranked_people as (
      select
        ...
        p.*,
        row_number() over (
          partition by l.survivor_id, p.full_name, coalesce(p.title,'')
          order by p.confidence desc nulls last, p.last_verified_at desc nulls last, p.updated_at desc, p.id
        ) as merge_rank
      from raven_people p
    ...
    copied_people as (
      ...
      select
        p.survivor_id, p.full_name, p.title, p.role_family, p.email, p.phone,
        p.source_url, p.source_type, p.confidence, p.last_verified_at,
        p.created_at, p.updated_at
      from ranked_people p
      where p.merge_rank=1
  ...
  const administrativeAliasRows = await sql.query(`
    with unresolved as (
      select
        a.id,
        a.state_code,
        regexp_replace(
          regexp_replace(
            ...
            '(school|schools|district|public|board|of|education|county|city)', '', 'g'
          ),
          '[^a-z0-9]+', '', 'g'
  ...
  return {
    consolidated: exactRows.length + normalizedRows.length + ravenPreservingRows.length + administrativeAliasRows.length,
    ...
    administrativeAliasToAuthoritative: administrativeAliasRows.length,
  };
}
