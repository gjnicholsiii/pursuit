...
const NCES_DIRECTORY_ZIP_FALLBACK = "https://nces.ed.gov/ccd/Data/zip/ccd_lea_029_2425_w_0a_051425.zip";

/**
 * CCD covers the 50 states, District of Columbia, and U.S.-related outlying
 * jurisdictions. Keep this registry national so reconciliation is not silently
 * limited to the 50-state subset.
 */
...
function directorySources(){
  const override=process.env.NCES_DIRECTORY_ZIP_URL?.trim();
  return [...new Set([override,NCES_DIRECTORY_ZIP_FALLBACK].filter((value):value is string=>Boolean(value)))];
}
...
async function fetchDirectoryZipWithRetry(attempts=4){
  ...
  for(const source of directorySources()){
    for(let attempt=1;attempt<=attempts;attempt++){
      try{
        const response=await fetch(source,{cache:"no-store",headers:{"user-agent":"Mozilla/5.0 PursuitGovernmentRevenue/1.0",accept:"application/zip,application/octet-stream,*/*"}});
    ...
    console.warn("NCES directory source failed",{source,error:lastError instanceof Error?lastError.message:String(lastError)});
  }
  throw new Error(`NCES directory fetch failed: ${lastError instanceof Error?lastError.message:String(lastError)}`);
}
...
async function fetchNationalDirectory(){
  ...
  directoryPromise=(async()=>{
    ...
    if(!byState.size)throw new Error("NCES directory CSV parsed zero jurisdiction rows");
...
async function fetchHtmlWithRetry(url:string,attempts=2){
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    try{
      const response=await fetch(url,{cache:"no-store",headers:{"user-agent":"Mozilla/5.0 PursuitGovernmentRevenue/1.0",accept:"text/html,application/xhtml+xml"}});
      ...
      return html;
    }catch(error){lastError=error;if(attempt<attempts)await sleep(500*attempt)}
  }
...
async function fetchPage(fips:string,page:number){
  ...
  $("div.resultRow").each((_,node)=>{
    const cells=$(node).children("div");if(cells.length<6)return;
    const anchor=$(cells[1]).find("a[href*='district_detail.asp']").first();
    const name=text(anchor.text());const href=anchor.attr("href")||"";
    const id=href.match(/[?&](?:ID2|DistrictID)=(\d+)/i)?.[1];if(!id||!name)return;
    const address=text($(cells[1]).find("span").first().text());
    const city=address.match(/,\s*([^,]+),\s*[A-Z]{2}\s+\d{5}/)?.[1]?.trim()||null;
    const countyText=text($(cells[3]).text());const county=/county$/i.test(countyText)?countyText:null;
    rows.push({ncesId:id,name,city,county,enrollment:parseNumber(text($(cells[4]).text())),schools:parseNumber(text($(cells[5]).text())),sourceUrl:new URL(href,url).toString()});
  });
  return{total,maxPages,rows:[...new Map(rows.map(r=>[r.ncesId,r])).values()]};
}

async function fetchStatePages(fips:string){
  const first=await fetchPage(fips,1);if(first.maxPages===1)return[first];
  const pages=[first];const remaining=Array.from({length:first.maxPages-1},(_,i)=>i+2);
  for(let i=0;i<remaining.length;i+=4)pages.push(...await Promise.all(remaining.slice(i,i+4).map(page=>fetchPage(fips,page))));
  return pages;
}
...
export async function syncNcesDistrictState(stateCode:string){
  const code=stateCode.toUpperCase();const fips=STATE_FIPS[code];if(!fips)throw new Error(`Unsupported jurisdiction ${stateCode}`);
  const stateData=await getStateRows(code,fips);const rows=stateData.rows;const sql=getSql();
  const payload=JSON.stringify(rows.map(row=>({nces_id:row.ncesId,name:row.name,city:row.city,county:row.county,source_url:row.sourceUrl})));
...
export async function syncNcesDistrictBatch(states:string[]):Promise<NcesDistrictSyncResult[]>{
  ...
  for(let i=0;i<states.length;i+=2){
    const settled=await Promise.all(states.slice(i,i+2).map(async stateCode=>{try{return await syncNcesDistrictState(stateCode)}catch(error){return{stateCode:stateCode.toUpperCase(),ncesTotal:0,rowsParsed:0,pages:0,inserted:0,updated:0,existing:0,error:error instanceof Error?error.message:String(error)}}}));
