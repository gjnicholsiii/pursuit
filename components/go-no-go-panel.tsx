...
export async function GoNoGoPanel({opportunityId,demo=false}:{opportunityId:string;demo?:boolean}){
  if(demo)return <section className="brief-panel"><div className="brief-panel-heading"><div><span>QUALIFICATION CHECK</span><h2>Ready for client review</h2></div></div><p className="brief-explainer">This opportunity cleared the four-state territory filter and was ranked against low-voltage capabilities. The live source, deadlines and package evidence remain available without changing Pursuit data during the demo.</p></section>;
